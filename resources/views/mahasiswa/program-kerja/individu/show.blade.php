...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8 flex justify-between items-start">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2">{{ $individuProgramKerja->judul }}</h1>
            <p class="text-gray-300">{{ ucfirst($individuProgramKerja->kategori) }} | {{ $individuProgramKerja->tanggal_mulai->format('d M Y') }} - {{ $individuProgramKerja->tanggal_selesai->format('d M Y') }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('program-kerja.edit-individu', $individuProgramKerja) }}" class="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700">Edit</a>
            <form action="{{ route('program-kerja.destroy-individu', $individuProgramKerja) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                @csrf @method('DELETE')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <p class="text-gray-700 whitespace-pre-wrap">{{ $individuProgramKerja->deskripsi }}</p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div id="luaran-form" class="hidden mb-6 p-4 bg-gray-50 border border-gray-300 rounded-lg">
                    <form action="{{ route('luaran.store-individu', $individuProgramKerja) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                        ...
                        @foreach ($luarans as $luaran)
                            <div class="p-4 border border-gray-200 rounded-lg">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <form action="{{ route('luaran.destroy', ['type' => 'individu', 'luaranId' => $luaran->id]) }}" method="POST" class="inline" onsubmit="return confirm('Yakin?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800 text-sm">Hapus</button>
                                    </form>
                                </div>
                                <p class="text-gray-700 text-sm mb-3">{{ Str::limit($luaran->deskripsi, 100) }}</p>
                                <form action="{{ route('luaran.update-status', ['type' => 'individu', 'luaranId' => $luaran->id]) }}" method="POST" class="space-y-2">
                                    @csrf @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                ...
                <div class="space-y-3 text-sm text-gray-700">
                    <div>
                        ...
                        <p>{{ $individuProgramKerja->lokasi }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Tanggal Mulai</p>
                        <p>{{ $individuProgramKerja->tanggal_mulai->format('d M Y') }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Tanggal Selesai</p>
                        <p>{{ $individuProgramKerja->tanggal_selesai->format('d M Y') }}</p>
                    </div>
                </div>
            </div>
