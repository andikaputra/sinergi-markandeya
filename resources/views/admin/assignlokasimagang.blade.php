...
@section('title', 'Penempatan Mahasiswa Magang')
...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('assign.lokasimagang.store') }}" method="POST" class="p-8">
            @csrf
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-slate-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-5 text-right">
                            <form action="{{ route('assign.lokasimagang.delete', $assignment->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm border border-red-100" onclick="return confirm('Hapus penempatan magang ini?')">
