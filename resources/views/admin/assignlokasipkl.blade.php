...
@section('title', 'Penempatan Mahasiswa PKL')
...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('assign.lokasipkl.store') }}" method="POST" class="p-8">
            @csrf
                            ...
                            @foreach($mahasiswas as $mahasiswa)
                                <label class="flex items-center p-4 bg-white border border-gray-100 rounded-2xl cursor-pointer hover:border-amber-300 hover:bg-amber-50 transition-all group">
                                    <div class="relative flex items-center justify-center">
                                        <input type="checkbox" name="nims[]" value="{{ $mahasiswa->nim }}" class="w-5 h-5 text-amber-600 border-gray-300 rounded-lg focus:ring-amber-500">
                                    </div>
                                    <div class="ml-4 flex-1">
                                        <p class="text-sm font-bold text-gray-800 group-hover:text-amber-700 transition-colors">{{ $mahasiswa->nama }}</p>
                                        <div class="flex items-center space-x-2 mt-0.5">
                                            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">{{ $mahasiswa->nim }}</span>
                                            <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                                            <span class="text-[10px] font-bold text-amber-500 uppercase tracking-tighter">{{ $mahasiswa->prodi }}</span>
                                        </div>
                                    </div>
                                </label>
                            @endforeach
                    ...
                    <tr class="hover:bg-slate-50/30 transition-colors group">
                        <td class="px-6 py-5">
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center font-bold text-xs">
                        ...
                        <td class="px-6 py-5">
                            <div class="flex items-center space-x-2 text-slate-600">
                                <i class="fas fa-building text-xs text-amber-500"></i>
                                <span class="text-sm font-bold">{{ $assignment->lokasipkl->nama_instansi }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-5 text-right">
                            <form action="{{ route('assign.lokasipkl.delete', $assignment->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm border border-red-100" onclick="return confirm('Hapus penempatan ini?')">
